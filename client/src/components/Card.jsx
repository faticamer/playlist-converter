    ...
    return (
        <div className='flex flex-col flex-wrap justify-center w-3/4 items-center m-6'>
            ...
            <div className='flex flex-col w-1/2 h-1/3 pt-10 my-10 justify-center items-center'>
                <h1 className={`${props.gradient} text-transparent text-3xl font-bold animate-gradient`}>Convert to {props.destination}</h1>
                <Link to={props.path} className={`mt-10 py-4 pr-32 pl-32 text-zinc-100 text-transparent text-xl ${props.border} font-bold text-center rounded-3xl ${props.hover}`}>Go!</Link>
            </div>
        </div>
    );
}
