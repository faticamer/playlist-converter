...
import {getLibrary, getTracksInfo} from '../service/server_calls'
...
const Convert = () => {
  ...
  if (!isValid) urlInputClass = styles.gradientBorderNeutral
    ...
    urlInputClass =
      'border border-gradientGreen rounded-xl delay-100 transition ease-in-out duration-500'

  let playlistNameInputClass = ''
  if (!isPresent) playlistNameInputClass = styles.gradientBorderNeutral
  else if (isPresent)
    playlistNameInputClass =
      'border border-gradientGreen rounded-xl delay-100 transition ease-in-out duration-500'

  let btnClass = ''
  if (isValid && isPresent)
    btnClass = styles.gradientBorderGreen + ' transition ease-in-out delay-100'
  else if (!isValid || !isPresent)
    btnClass =
      'border rounded-xl border-zinc-400 transition ease-in-out duration-50 hover:border-zinc-100'
                  ...
                  <div>
                    {isLoading ? (
                      <div className="flex flex-col justify-center items-center">
                        ...
                        {isMobile ? (
                          <span className={styles.mobileLoader} />
                        ) : (
                          <></>
                        )}
                      </div>
